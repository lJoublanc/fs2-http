...
object ChunkedEncoding {
  ...
  def decode[F[_]](maxChunkHeaderSize:Int):Pipe[F, Byte, Byte] = {
    ...
    def go(expect:Either[ByteVector,Long]): Stream[F,Byte] => Pull[F,Byte, Unit] = { 
            ...
            case Left(header) =>
              ...
              if (endOfheader == 0) go(expect)(h cons byteVector2Chunk(bv.drop(`\r\n`.size))) //strip any leading crlf on header, as this starts with /r/n
                ...
                readChunkedHeader(hdr.dropRight(`\r\n`.size)) match {
                  ...
                  case Some(sz) => go(Right(sz))(h cons byteVector2Chunk(rem))
                }
              }

            case Right(remains) =>
              if (remains == bv.size) Pull.output(byteVector2Chunk(bv)) >> go(Left(ByteVector.empty))(h)
              else if (remains > bv.size) Pull.output(byteVector2Chunk(bv)) >> go(Right(remains - bv.size))(h)
              else {
                val (out,next) = bv.splitAt(remains.toInt)
                Pull.output(byteVector2Chunk(out)) >> go(Left(ByteVector.empty))(h cons Chunk.seq(next.toSeq))
              }
          }
        case None => Pull.done
      }
    }
    in => go(Left(ByteVector.empty))(in).stream
  }


  private val lastChunk: Chunk[Byte] = byteVector2Chunk((ByteVector('0') ++ `\r\n` ++ `\r\n`).compact)

  /**
    * Encodes chunk of bytes to http chunked encoding.
    */
  def encode[F[_]]:Pipe[F,Byte,Byte] = {
    def encodeChunk(bv:ByteVector):Chunk[Byte] = byteVector2Chunk {
