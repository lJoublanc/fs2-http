...
object MatcherSpec extends Properties("Matcher"){
  ...
  implicit class RouteTaskInstance(val self: Route[IO]) {
    def matches(request: HttpRequestHeader, body: Stream[IO, Byte] = Stream.empty): MatchResult[IO, Stream[IO, HttpResponse[IO]]] =
      Matcher.run(self)(request, body).unsafeRunSync()
  }

  val RespondOk: Stream[IO, HttpResponse[IO]] = Stream.emit(HttpResponse[IO](HttpStatusCode.Ok))
